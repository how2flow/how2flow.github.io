...
(pca9685) $ sudo python3 -m pip install wheel
```
<br>
...
(pca9685) $ sudo ./autogen.sh --enable-tools=yes --prefix=/usr/local/ --enable-bindings-python CFLAGS="-I/$include_path"
(pca9685) $ sudo make
(pca9685) $ sudo ldconfig
(pca9685) $ sudo cp bindings/python/.libs/gpiod.* /usr/lib/python3.?/dist-packages/
```
<br>
...
(pca9685) $ cd Adafruit_CircuitPython_PCA9685
(pca9685) $ sudo python3 -m pip install -r requirements.txt
(pca9685) $ vi examples/pca9685_simpletest.py
```
...
수정 완료 후 실행합니다.<br>
```
(pca9685) $ sudo python3 examples/pca9685_simpletest.py
```
